...
def PCA(vcf,metadata_path,run,path,sample_id,classification):
        ...
        plink.write("#!/bin/bash")
        plink.write('\n')
        plink.write("#SBATCH --time=1:00:00   # walltime limit (HH:MM:SS)")
        plink.write('\n')
        plink.write("#SBATCH --ntasks-per-node=7")
        plink.write('\n')
        plink.write("#SBATCH -p RM-shared")
        plink.write('\n')
...
rule Intersect:
    ...
    shell:
        ...
        tabix -h -T data/Chr{wildcards.chr}_SNPs.bed {wildcards.wd}/{wildcards.sample}/Chr{wildcards.chr}.vcf.gz | bgzip  > {wildcards.wd}/{wildcards.sample}/Chr{wildcards.chr}.comm_pos.vcf.gz
