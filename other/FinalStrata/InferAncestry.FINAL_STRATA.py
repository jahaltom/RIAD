...
rule SuperPop:
    ...
    run:
        ...
        #Read in metadata
        tx_metadata=pd.read_csv("Transcript_level_metadata.tsv",sep='\t')
        #Extract specific genes.
        ebReg=tx_metadata[tx_metadata["EB_type"]=="EB_novel"]
        cov=tx_metadata[tx_metadata["EB_type"]=="COVID-19 Related EB_novel"]
        ann=tx_metadata[tx_metadata["Gene_type"]!="EB_novel"]
        tx_metadata=pd.concat([ebReg,cov,ann])
