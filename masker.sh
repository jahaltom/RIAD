...
bcftools view -s SAMPLE --threads 7 --output-type z OUTPUT/SAMPLE/ChrAll_.SAMPLE.vcf.gz > OUTPUT/SAMPLE/SAMPLE.vcf.gz
...
bcftools annotate --set-id '%CHROM\:%POS\:%REF\:%ALT' --threads 7 OUTPUT/SAMPLE/SAMPLE.vcf.gz > OUTPUT/SAMPLE/SAMPLE.vcf
...
bcftools annotate -x ID,INFO --threads 7  --output-type z OUTPUT/SAMPLE/Temp_ChrAll_Noise.SAMPLE.vcf.gz  > OUTPUT/SAMPLE/ChrAll_Noise.SAMPLE.vcf.gz
...
bcftools view -s ^SAMPLE --threads 7  --output-type z OUTPUT/SAMPLE/ChrAll_.SAMPLE.vcf.gz  > OUTPUT/SAMPLE/ChrAll_.1KGP.SAMPLE.vcf.gz
bcftools index -t OUTPUT/SAMPLE/ChrAll_.1KGP.SAMPLE.vcf.gz

bcftools merge OUTPUT/SAMPLE/ChrAll_.1KGP.SAMPLE.vcf.gz OUTPUT/SAMPLE/ChrAll_Noise.SAMPLE.vcf.gz --output-type z --threads 7 > OUTPUT/SAMPLE/ChrAll.SAMPLE.Noise.vcf.gz
